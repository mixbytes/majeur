...
- **Ghost sum variables with Sload hooks**: `Shares` and `Loot` use ghost `mathint` variables with `Sstore` hooks to track the sum of all balances. An `Sload` hook on `balanceOf` constrains individual balances to be bounded by the ghost sum, eliminating false counterexamples from the prover's arbitrary state exploration
