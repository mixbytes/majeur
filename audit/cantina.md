...
>   - **MAJEUR-7 (Medium):** DAICO LP drift cap uses `tribForLP` instead of `totalTrib` — the comment cites the correct formula but the code uses the wrong variable, leaking extra sale inventory when pool spot > OTC rate. Not a blocker: only triggers with LP-enabled DAICO sales when the pool drifts above OTC, and the impact is bounded (~33% extra in worst case). Genuine math bug in DAICO.sol — most actionable fix in this audit.
...
| MAJEUR-7 | DAICO LP drift cap math bug | Medium | Arithmetic | New |
